...
class MusicApplication(Frame):
    ...
    def __init__(self, master):
        ...
        self.locations = ['']   #locations (VERANDEREN PLS)
        ...
        self.track_pool.load_dir(self.locations[0]) #MOET WEG
        ...
        self.tracklistListBox = Treeview(master) #ttk Treeview
        ...
        self.update_tracklistbox() #inserts items into tracklistbox

        #adding a scrollbar to current_tracklist Listbox
        self.tracklistListBox_scrollbar = Scrollbar(master, orient='vertical', command=self.tracklistListBox.yview) #tkinter Scrollbar
        self.tracklistListBox_scrollbar.pack()

        #edit tags_button
        self.edit_tagsButton = Button(frame, text="Edit tags") #tkinter Button
        ...
        #initialize player
        tkSnack.initializeSnack(master) #standard init process tkSnack
        self.soundObject = tkSnack.Sound() #create a SoundObject (required by tkSnack)

        #button to add selected tracks to queue
        self.add_to_queue_button = Button(frame, text="Add selected to queue") #create Tkinter Button
        ...
        self.add_everything_to_queue = Button(frame, text="Add everything to queue") #create Tkinter Button
        ...
        self.add_to_playlist_button = Button(frame, text='add to playlist') #create Tkinter Button
        ...
        self.full_library_button = Button(frame, text='show full library') #create Tkinter Button
        ...
        self.remove_tracks_from_playlist_button = Button(frame, text="Remove selected tracks from playlist") #create Tkinter Button
        ...
        self.fileMenu.add_command(label="Exit", command=None) #command to quit the application
        ...
        self.playlistsBox = Treeview(master) #ttk TreeView
        ...
        #create queue to be used with the queueBox
        self.queue = []
    ...
    def player_window_go(self):
        if self.player_window is None: #check if there is no player window already
            ...
            Label(self.player_window, textvariable=self.queueBox_count).pack() #link counter to label (self-updating)

            #play button on player_window
            self.playButton = Button(self.player_window, text="Play") #create button on player_window
            self.playButton.bind("<Button-1>", self.playSound) #bind button to playSound function
            self.playButton.pack(side=LEFT)

            #stop button on player_window
            self.stopButton = Button(self.player_window, text="Stop") #create button on player_window
            self.stopButton.bind("<Button-1>", self.stopSound) #bind button to stopSound function
            self.stopButton.pack(side=LEFT)

            #pause button on player_window
            self.pauseButton = Button(self.player_window, text="Pause") #create button on player_Window
            ...
            self.songprogressBar = Scale(self.player_window, from_=0, to=128, command=None) #tkinter Scale
            self.songprogressBar.pack()

            #add a bar to be able to set the volume
            self.volumeBar = Scale(self.player_window, from_=0, to=100, orient=VERTICAL, command=self.update_volume) #tkinter Scale
            self.volumeBar.set(50) #set init value
            self.volumeBar.pack()

            #volume text variable
            self.volume = StringVar()
            Label(self.player_window, textvariable=self.volume).pack() #label binded to self.volume
            self.volume.set(str(int(tkSnack.audio.play_gain())) + "%") #set volumetext

            #create button to go to next song from queue
            self.nextButton = Button(self.player_window, text="Next") #tkinter Button
    ...
    #function that is exectued when the selection of playlists is changed
    ...
    def select_library_locations_window(self, event=None):
        ...
        listbox = Listbox(self.window) #create new listbox
        ...
        Button(self.window, text='askopenfile', command=self.askdirectory).pack(side=LEFT) #binded to askdir function
    ...
    def askdirectory(self):
        ...
        self.locations.append(directory) #append the selected directory to all locations
        self.track_pool.load_dir(directory) #load the new directory into the track_pool
        self.track_pool.update_track_pool() #update the trackpool
        self.current_tracklist.reset() #reset the current_tracklist
        self.update_tracklistbox() #update the tracklistbox
        self.window.destroy() #destroy the directory window
